...
# datasets related
from lib.train.dataset import EventVOT # COESOT, FE108, VisEvent
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", "COESOT", "COESOT_VAL", "FE108", "FE108_VAL", "VisEvent", "VisEvent_VAL","EventVOT"]
        # if name == "LASOT":
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if "backbone_s" not in n and p.requires_grad]},
            {
                "params": [p for n, p in net.named_parameters() if "backbone_s" in n and p.requires_grad],
