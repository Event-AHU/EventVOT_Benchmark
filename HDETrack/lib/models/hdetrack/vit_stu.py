...
class VisionTransformerCE_S(VisionTransformer):
    ...
    def forward_features(self, z, x,
        ...
        for i, blk in enumerate(self.blocks):
            x, global_index_t, global_index_s, removed_index_s, s_attn = \
                blk(x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate)
            if self.ce_loc is not None and i in self.ce_loc:
                removed_indexes_s.append(removed_index_s)

        x = self.norm(x)    ## student [bs,320,768]

        # lens_x_new = global_index_s.shape[1]
        # lens_z_new = global_index_t.shape[1]

        # z = x[:, :lens_z_new]       # [16, 64, 768]
        # x = x[:, lens_z_new:]       # [16, 256, 768]
    ...
    def forward(self, z, x,
        ...
        x, s_attn = self.forward_features(z, x, 
