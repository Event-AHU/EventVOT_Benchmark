...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, x,                  #  z       = [32, 3, 256, 256]  x       = [32, 3, 128, 128]
                         event_z, event_x,            #  event_z = [32, 1, 19, 1024]  event_x = [32, 1, 19, 4096]
        ...
        event_z = self.pos_embed_event(event_z)         # [32, 64,  768]   
        event_x = self.pos_embed_event(event_x)         # [32, 256, 768]
        z = self.patch_embed(z)                     # [32, 64,  768]  
        x = self.patch_embed(x)                     # [32, 256, 768]

        event_z += self.pos_embed_z                 # [32, 64,  768]
        event_x += self.pos_embed_x                 # [32, 256, 768]
        z += self.pos_embed_z                       # [32, 64,  768]
        x += self.pos_embed_x                       # [32, 256, 768]
        ...
        x = torch.cat((z, event_z, x, event_x),dim=1)      # teacher 64+64+256+256=640
        # e_x = torch.cat([event_z, event_x], dim=1)       # student 64+256=320
        if self.add_cls_token:
        ...
        cat_x = self.norm(x)                    ## teacher 特征  [32, 640, 768]
         
        lens_x_new = global_index_s.shape[1]    # 256
        lens_z_new = global_index_t.shape[1]    # 64

        z = cat_x[:, :lens_z_new*2]             # [32, 128, 768] z+event_z
        x = cat_x[:, lens_z_new*2:]             # [32, 512, 768] x+event_x
    ...
    def forward(self, z, x, event_z, event_x,
        ...
        x, cat_x, attn = self.forward_features( z, x, event_z, event_x,
